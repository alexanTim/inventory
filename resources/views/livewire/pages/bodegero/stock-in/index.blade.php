...
<div class="min-h-[70vh] flex flex-col justify-center items-center py-4">
    ...
    <div class="w-full max-w-md mx-auto space-y-4" wire:init="ensureCorrectPO">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ $foundPurchaseOrder->supplyOrders->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            ...
            <div class="space-y-4">
                @foreach($foundPurchaseOrder->supplyOrders as $supplyOrder)
                <div class="border border-gray-200 dark:border-zinc-600 rounded-xl p-4 space-y-3">
                    <!-- Item Header -->
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <h4 class="font-semibold text-gray-900 dark:text-white text-sm">{{ $supplyOrder->supplyProfile->supply_description }}</h4>
                            <p class="text-xs text-gray-600 dark:text-gray-400">SKU: {{ $supplyOrder->supplyProfile->supply_sku }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $supplyOrder->order_qty }} {{ $supplyOrder->supplyProfile->supply_uom }}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex space-x-2">
                            <label class="flex items-center">
                                <input type="radio" 
                                       wire:model="itemStatuses.{{ $supplyOrder->id }}" 
                                       value="good" 
                    ...
                    <!-- Remarks -->
                    <div class="space-y-2">
                        <label for="remarks-{{ $supplyOrder->id }}" class="text-xs font-medium text-gray-700 dark:text-gray-300">Remarks:</label>
                        <textarea 
                            id="remarks-{{ $supplyOrder->id }}"
                            wire:model="itemRemarks.{{ $supplyOrder->id }}"
                            rows="2"
                    ...
                    @if($supplyOrder->supplyProfile->itemClass && $supplyOrder->supplyProfile->itemClass->name === 'consumable')
                    <div class="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg"> 
                        <div class="space-y-3">
                            <div>
                                <label for="expiry-{{ $supplyOrder->id }}" class="text-xs font-medium text-amber-700 dark:text-amber-300">Expiration Date:</label>
                                <input 
                                    type="date"
                                    id="expiry-{{ $supplyOrder->id }}"
                                    wire:model="itemExpirationDates.{{ $supplyOrder->id }}"
                                    class="w-full px-2 py-1 text-xs border border-amber-300 rounded focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-100 dark:focus:ring-amber-400 dark:focus:border-amber-400">
                            </div>

                            <!-- Manufactured Date -->
                            <div>
                                <label for="mfg-{{ $supplyOrder->id }}" class="text-xs font-medium text-amber-700 dark:text-amber-300">Manufactured Date:</label>
                                <input 
                                    type="date"
                                    id="mfg-{{ $supplyOrder->id }}"
                                    wire:model="itemManufacturedDates.{{ $supplyOrder->id }}"
                                    class="w-full px-2 py-1 text-xs border border-amber-300 rounded focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-100 dark:focus:ring-amber-400 dark:focus:border-amber-400">
                            </div>

                            <!-- Location -->
                            <div>
                                <label for="location-{{ $supplyOrder->id }}" class="text-xs font-medium text-amber-700 dark:text-amber-300">Storage Location:</label>
                                <select 
                                    id="location-{{ $supplyOrder->id }}"
                                    wire:model="itemLocations.{{ $supplyOrder->id }}"
                                    class="w-full px-2 py-1 text-xs border border-amber-300 rounded focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-100 dark:focus:ring-amber-400 dark:focus:border-amber-400">
                            ...
                            <div>
                                <label 
                                    for="received-qty-{{ $supplyOrder->id }}"
                                    class="text-xs font-medium text-amber-700 dark:text-amber-300">Received Quantity:
                                </label>
                                <input 
                                    type="number"
                                    min="1"
                                    id="received-qty-{{ $supplyOrder->id }}"
                                    wire:model="received_qty.{{ $supplyOrder->id }}"
                                    class="w-full px-2 py-1 text-xs border border-amber-300 rounded focus:ring-2 focus:ring-amber-500 focus:border-amber-500 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-100 dark:focus:ring-amber-400 dark:focus:border-amber-400"
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            ...
            <div class="bg-blue-100 dark:bg-zinc-800 rounded-xl p-4 mb-6 border border-blue-200 dark:border-zinc-700">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-bold text-blue-900 dark:text-white">{{ $foundPurchaseOrder->supplyOrders->count() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="border border-gray-200 dark:border-zinc-700 rounded-xl p-4 space-y-3 bg-white dark:bg-zinc-800">
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <h4 class="font-bold text-gray-900 dark:text-white text-sm">{{ $supplyOrder->supplyProfile->supply_description }}</h4>
                            <p class="text-xs text-gray-700 dark:text-gray-200">SKU: {{ $supplyOrder->supplyProfile->supply_sku }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $supplyOrder->order_qty }} {{ $supplyOrder->supplyProfile->supply_uom }}</p>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if(isset($itemStatuses[$supplyOrder->id]))
                            @if($itemStatuses[$supplyOrder->id] === 'good')
                            ...
                            @elseif($itemStatuses[$supplyOrder->id] === 'incomplete')
                            ...
                            @elseif($itemStatuses[$supplyOrder->id] === 'destroyed')
                    ...
                    <!-- Remarks -->
                    @if(isset($itemRemarks[$supplyOrder->id]) && !empty($itemRemarks[$supplyOrder->id]))
                    <div class="bg-gray-100 dark:bg-zinc-900 rounded-lg p-3 border border-gray-200 dark:border-zinc-700">
                        ...
                        <p class="text-sm text-gray-900 dark:text-white">{{ $itemRemarks[$supplyOrder->id] }}</p>
                    </div>
                    @endif
                </div>
                @endforeach
