...
<div class="min-h-[70vh] flex flex-col justify-center items-center py-4">
        ...
        <div class="flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 rounded-2xl shadow-2xl p-4 border-b border-gray-200 dark:border-zinc-700 z-20">
            @foreach($steps as $i => $step)
                <div class="flex-1 flex flex-col items-center relative">
                    ...
                    @else
                        <div class="w-9 h-9 flex items-center justify-center rounded-full bg-zinc-300 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-300 text-base shadow-md">
                            <span class="font-bold">{{ $i + 1 }}</span>
                        </div>
                    @endif
                    <!-- Connecting Line -->
                    ...
                    <div class="mt-2 text-center">
                        <div class="text-sm font-bold {{ $currentStep === $i ? 'text-blue-900 dark:text-blue-100' : 'text-zinc-700 dark:text-zinc-200' }}">
                            {{ $step['label'] }}
                        </div>
                    </div>
                </div>
    ...
    <div id="camera-status-toast" class="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-xs px-4 hidden">
        <div id="camera-status-toast-content" class="flex items-start gap-3 p-4 rounded-xl shadow-xl border bg-blue-50 border-blue-300 text-blue-900 dark:bg-blue-900 dark:border-blue-700 dark:text-blue-100">
            ...
            <button onclick="document.getElementById('camera-status-toast').classList.add('hidden')" class="ml-2 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 focus:outline-none">
    ...
    <div class="w-full max-w-md mx-auto space-y-4" wire:init="ensureCorrectPO">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-bold text-zinc-900 dark:text-white">Shipment Details</h2>
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-zinc-600 dark:text-zinc-400">Shipping Plan Number:</span>
                    <span class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $foundShipment->shipping_plan_num }}</span>
                </div>              
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-zinc-600 dark:text-zinc-400">Shipment Created Date:</span>
                    <span class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $foundShipment->created_at->format('M d, Y') }}</span>
                </div>               
                <div class="flex justify-between">
                    <span class="text-sm font-medium text-zinc-600 dark:text-zinc-400">Total Items:</span>
                    <span class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $foundShipment->salesOrder->items->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            <h3 class="text-lg font-bold text-zinc-900 dark:text-white mb-4">Review Items</h3>
            ...
            <div class="space-y-4">
                ...
                <div class="border border-gray-200 dark:border-zinc-600 rounded-xl p-4 space-y-3">
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <h4 class="font-semibold text-zinc-900 dark:text-white text-sm">{{ $item->product->supply_description }}</h4>
                            <p class="text-xs text-zinc-600 dark:text-zinc-400">SKU: {{ $item->product->supply_sku }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ $item->order_qty }} {{ $item->product->supply_uom }}</p>
                            <p class="text-xs text-zinc-600 dark:text-zinc-400">Ordered</p>
                        </div>
                    </div>

                    <!-- Status Selection -->
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Item Status:</label>
                        <div class="flex space-x-2">
                            <label class="flex items-center">
                                <input type="radio" 
                                       wire:model="itemStatuses.{{ $item->id }}" 
                                       value="good" 
                                       class="w-4 h-4 text-green-600 bg-zinc-100 border-zinc-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-zinc-800 focus:ring-2 dark:bg-zinc-700 dark:border-zinc-600">
                                <span class="ml-2 text-xs text-zinc-700 dark:text-zinc-300">Good</span>
                            </label>
                            <label class="flex items-center">
                                <input type="radio" 
                                       wire:model="itemStatuses.{{ $item->id }}" 
                                       value="incomplete" 
                                       class="w-4 h-4 text-yellow-600 bg-zinc-100 border-zinc-300 focus:ring-yellow-500 dark:focus:ring-yellow-600 dark:ring-offset-zinc-800 focus:ring-2 dark:bg-zinc-700 dark:border-zinc-600">
                                <span class="ml-2 text-xs text-zinc-700 dark:text-zinc-300">Incomplete</span>
                            </label>
                            <label class="flex items-center">
                                <input type="radio" 
                                       wire:model="itemStatuses.{{ $item->id }}" 
                                       value="destroyed" 
                                       class="w-4 h-4 text-red-600 bg-zinc-100 border-zinc-300 focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-zinc-800 focus:ring-2 dark:bg-zinc-700 dark:border-zinc-600">
                                <span class="ml-2 text-xs text-zinc-700 dark:text-zinc-300">Destroyed</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- Remarks -->
                    <div class="space-y-2">
                        <label for="remarks-{{ $item->id }}" class="text-xs font-medium text-zinc-700 dark:text-zinc-300">Remarks:</label>
                        <textarea 
                            id="remarks-{{ $item->id }}"
            ...
            <div class="mt-8 space-y-3">
                <label for="general-remarks" class="text-base font-medium text-zinc-700 dark:text-zinc-300">General Remarks:</label>
                <textarea 
                    id="general-remarks"
    ...
    <div class="w-full max-w-md mx-auto space-y-4">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-6 border border-gray-200 dark:border-zinc-700">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-bold text-zinc-900 dark:text-white">Shipment Report</h2>
            ...
            <div class="space-y-4">
                <h3 class="text-lg font-bold text-zinc-900 dark:text-white">Items Review Summary</h3>
                ...
                <div class="border border-gray-200 dark:border-zinc-700 rounded-xl p-4 space-y-3 bg-white dark:bg-zinc-800">
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <h4 class="font-bold text-zinc-900 dark:text-white text-sm">{{ $item->product->supply_description }}</h4>
                            <p class="text-xs text-zinc-700 dark:text-zinc-200">SKU: {{ $item->product->supply_sku }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-bold text-zinc-900 dark:text-white">{{ $item->order_qty }} {{ $item->product->supply_uom }}</p>
                            <p class="text-xs text-zinc-700 dark:text-zinc-200">Ordered</p>
                        </div>
                    </div>

                    <!-- Status Badge -->
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-bold text-zinc-900 dark:text-white">Status:</span>
                    ...
                    <div class="bg-gray-100 dark:bg-zinc-900 rounded-lg p-3 border border-gray-200 dark:border-zinc-700">
                        <p class="text-xs text-zinc-900 dark:text-white mb-1">Remarks:</p>
                        <p class="text-sm text-zinc-900 dark:text-white">{{ $itemRemarks[$item->id] }}</p>
                    </div>
                    @endif
                </div>
                @endforeach
            ...
            <div class="mt-6 bg-gray-100 dark:bg-zinc-900 rounded-xl p-4 border border-gray-200 dark:border-zinc-700">
                <h4 class="text-sm font-bold text-zinc-900 dark:text-white mb-2">General Remarks:</h4>
                <p class="text-sm text-zinc-900 dark:text-white">{{ $generalRemarks }}</p>
            </div>
            @endif
    ...
    <div class="w-full max-w-md mx-auto space-y-4">
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-8 border border-gray-200 dark:border-zinc-700 text-center">
            ...
            <!-- Success Message -->
            <h2 class="text-2xl font-bold text-zinc-900 dark:text-white mb-4">Report Submitted!</h2>
            ...
            <div class="mb-6 flex flex-col items-center gap-1">
                <div class="text-base font-bold text-zinc-900 dark:text-white">Scanned by:
                    <span class="font-semibold">{{ auth()->user()->name }}</span>
                </div>
                <div class="text-base font-bold text-zinc-900 dark:text-white">Received Date:
