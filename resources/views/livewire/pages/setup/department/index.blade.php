...
<x-slot:subheader>Department Management</x-slot:subheader>
<div class="">
    <div class="">
        @can(\App\Enums\Enum\PermissionEnum::CREATE_DEPARTMENTS->value)
        <x-collapsible-card title="Create Department" open="false" size="full">
            <form wire:submit.prevent="create" x-show="open" x-transition>
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        <x-input type="text" wire:model="name" name="name" label="Department Name" placeholder="Enter department name" />
                    </div>
                    <div>
        ...
        <x-collapsible-card title="Departments List" open="true" size="full">
            <div class="flex items-center justify-between p-4 pr-10">
                <div class="flex space-x-6">
                    <div class="relative">
                        <x-input type="text" wire:model.live="search" name="search" label="Search" placeholder="Search departments..." />
                    </div>
                </div>
            </div>
                        ...
                        <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    @can(\App\Enums\Enum\PermissionEnum::EDIT_DEPARTMENTS->value)
                                    <x-button type="button" wire:click="edit({{ $department->id }})" variant="warning" size="sm">Edit</x-button>
                                    @endcan
                                    @can(\App\Enums\Enum\PermissionEnum::DELETE_DEPARTMENTS->value)
                                    <x-button type="button" wire:click="confirmDelete({{ $department->id }})" variant="danger" size="sm">Delete</x-button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                ...
                <div class="relative w-full max-w-2xl max-h-full">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                            <x-button type="button" wire:click="{{ $editingDepartmentId ? 'update' : 'create' }}" variant="primary">{{ $editingDepartmentId ? 'Save changes' : 'Submit' }}</x-button>
