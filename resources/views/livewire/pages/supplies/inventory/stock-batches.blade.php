        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <div class="text-gray-600 dark:text-gray-300 mb-1">Class: 
                    @if($supply->itemClass)
                        @if($supply->itemClass->name === 'consumable')
                        ...
                        @else
                            <span class="font-semibold">{{ $supply->itemClass->name }}</span>
                        @endif
                    @else
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                                        ...
                                        @if($batch->expiration_date)
                                            <div class="flex flex-col">
                                                ...
                                                @if($batch->getDaysUntilExpiry() !== null)
                                                                                                    @if($supply->itemClass && $supply->itemClass->name === 'consumable' && $batch->isExpired())
                                                    <span class="text-xs text-red-600 font-medium">Expired {{ abs($batch->getDaysUntilExpiry()) }} days ago</span>
                                                @elseif($supply->itemClass && $supply->itemClass->name === 'consumable' && $batch->isExpiringSoon())
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $supply->itemClass && $supply->itemClass->name === 'consumable' ? '7' : '6' }}" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        @if($supply->itemClass && $supply->itemClass->name === 'consumable')
                                            <div>
                                                ...
                                                <input type="date" wire:model="expiration_date" 
                                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
