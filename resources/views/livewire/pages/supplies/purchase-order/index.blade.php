...
<div class="">
            ...
            <div>
                ...
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <div class="flex items-center justify-between p-4 pr-10">
                        ...
                        @if(
                            auth()->user()->can(\App\Enums\Enum\PermissionEnum::CREATE_SUPPLY_PURCHASE_ORDER->value) 
                        )                        
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left rtl:text-right text-zinc-500 dark:text-zinc-400 border border-zinc-300 dark:border-zinc-700">
                            <thead class="text-sm text-zinc-700 uppercase bg-zinc-50 dark:bg-zinc-700 dark:text-zinc-400 border-b border-zinc-300 dark:border-zinc-700">
                                <tr class="border-b border-zinc-300 dark:border-zinc-700">
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">QR Code</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">PO #</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Supplier</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Status</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Order Date</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Receiving department </th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Total Qty</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Total Price</th>
                                    <th scope="col" class="px-6 py-3 border-b border-zinc-300 dark:border-zinc-700">Action</th>
                                </tr>
                            </thead>
                            <tbody class="border-t border-zinc-300 dark:border-zinc-700">
                                @forelse($purchaseOrders as $po)
                                    <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-zinc-50 even:dark:bg-zinc-800 border-b border-zinc-300 dark:border-zinc-700">
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">
                                            <a href="{{ route('supplies.PurchaseOrder.showWithQr', ['Id' => $po->id]) }}"
                                                class="inline-block cursor-pointer hover:opacity-80 transition-opacity"
                                                title="Click to view PO details with QR code">
                                                {!! QrCode::size(70)->generate(json_encode([
                                                    'type' => 'purchase_order',
                                                    'po_num' => $po->po_num,
                                                    'status' => $po->status,
                                                    'supplier' => $po->supplier ? $po->supplier->name : 'N/A',
                                                    'department' => $po->department ? $po->department->name : 'N/A',
                                    ...
                                    'system' => 'Gentle Walker PO System'
                                        ...
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ $po->po_num }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ $po->supplier->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">
                                            <span
                                        ...
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ $po->order_date->format('M d, Y') }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ $po->department->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ number_format($po->total_qty, 2) }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">{{ number_format($po->total_price, 2) }}</td>
                                        <td class="px-6 py-4 border-b border-zinc-300 dark:border-zinc-700">
                                            <div class="flex space-x-2">
                                                @if(
                                                    auth()->user()->can(\App\Enums\Enum\PermissionEnum::VIEW_SUPPLY_PURCHASE_ORDER->value) 
                                                )
                                                    <x-button href="{{ route('supplies.PurchaseOrder.showWithQr', ['Id' => $po->id]) }}"
                                                        class="" variant="secondary">
                                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V6a1 1 0 00-1-1H5a1 1 0 00-1 1v1a1 1 0 001 1zm12 0h2a1 1 0 001-1V6a1 1 0 00-1-1h-2a1 1 1 0 001 1zM5 20h2a1 1 0 001-1v-1a1 1 0 00-1-1H5a1 1 0 00-1 1v1a1 1 0 001 1z"></path>
                                                        </svg>
                                                        View 
                                                    </x-button>
                                                @endif
                                                ...
                                                @if(
                                                    auth()->user()->can(\App\Enums\Enum\PermissionEnum::EDIT_SUPPLY_PURCHASE_ORDER->value) 
                                                )
                                                ...
                                                @if(
                                                    auth()->user()->can(\App\Enums\Enum\PermissionEnum::DELETE_SUPPLY_PURCHASE_ORDER->value) 
                                                )
                                ...
                                @empty
                                    <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-zinc-50 even:dark:bg-zinc-800 border-b border-zinc-300 dark:border-zinc-700">
                                        <td colspan="9" class="px-6 py-4 text-center border-b border-zinc-300 dark:border-zinc-700">No purchase orders found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div x-data="{ show: @entangle('showQrModal') }" 
        ...
        <div class="relative w-full max-w-4xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" @click.stop x-show="show && $wire.selectedPurchaseOrder">
                        ...
                        <div class="flex flex-col items-center justify-center p-6 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            ...
                            <div class="bg-white p-4 rounded-lg shadow-sm">
                                {!! QrCode::size(200)->generate(json_encode([
                                    'type' => 'purchase_order',
                                    'po_num' => $selectedPurchaseOrder->po_num,
                                    'status' => $selectedPurchaseOrder->status,
                                    'supplier' => $selectedPurchaseOrder->supplier ? $selectedPurchaseOrder->supplier->name : 'N/A',
                                    'department' => $selectedPurchaseOrder->department ? $selectedPurchaseOrder->department->name : 'N/A',
                                    'total_qty' => $selectedPurchaseOrder->total_qty,
                                    'total_price' => $selectedPurchaseOrder->total_price,
                                    'order_date' => $selectedPurchaseOrder->order_date->format('Y-m-d'),
                                    'system' => 'Gentle Walker PO System'
                                ])) !!}
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 font-mono">{{ $selectedPurchaseOrder->po_num }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 text-center">Enhanced QR code for stock-in scanning</p>
                        </div>
