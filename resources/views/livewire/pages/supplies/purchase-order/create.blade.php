        ...
        <x-collapsible-card title="Purchase Order Details" open="true" size="full">
            <form wire:submit.prevent="submit">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <x-dropdown wire:model="payment_terms" name="payment_terms" label="Payment Terms" :options="[
                        ...
                        'Net 60' => 'Net 60',
                        'Net 90' => 'Net 90',
                        '30% Down, 70% on Delivery' => '30% Down, 70% on Delivery',
                        '50% Down, 50% on Delivery' => '50% Down, 50% on Delivery',
                        'Advance Payment' => 'Advance Payment',
                        'Letter of Credit (LC)' => 'Letter of Credit (LC)',
                        'Post Dated Check (PDC)' => 'Post Dated Check (PDC)',
                        'End of Month (EOM)' => 'End of Month (EOM)',
                        'Monthly' => 'Monthly',
                    ]" placeholder="Select payment terms" />
                    <x-input type="text" wire:model="quotation" name="quotation" label="Quotation" placeholder="Enter quotation reference" />
                    
                    {{-- <x-input type="text" wire:model="approver" name="approver" label="Approver" placeholder="Enter approver name" /> --}}
                </div>
            </form>
        </x-collapsible-card>
                    ...
                    <div x-data="{ show: @entangle('showModal') }" 
                        ...
                        <div class="relative w-full max-w-2xl max-h-full">
                            ...
                            <div class="relative">
                                ...
                                <x-input type="number" step="0.01" wire:model="unit_price" name="unit_price" label="Unit Price" placeholder="Unit price will be set automatically" readonly="true" />
                            ...
                            <x-button type="button" wire:click="closeModal" variant="secondary">
                                ...
                                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                                    <button type="button" wire:click="addItem" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        {{ $editingItemIndex !== null ? 'Update Item' : 'Add Item' }}
                                    </button>
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border border-gray-300 dark:border-gray-700">
                            <thead class="text-sm text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 border-b border-gray-300 dark:border-gray-700">
                                <tr class="border-b border-gray-300 dark:border-gray-700">
                                    <th scope="col" class="px-6 py-3 border-b border-gray-300 dark:border-gray-700">Item</th>
                                    <th scope="col" class="px-6 py-3 border-b border-gray-300 dark:border-gray-700">Type</th>
                                    <th scope="col" class="px-6 py-3 border-b border-gray-300 dark:border-gray-700">Unit Price</th>
                                    <th scope="col" class="px-6 py-3 border-b border-gray-300 dark:border-gray-700">Quantity</th>
                            ...
                            <tbody class="border-t border-gray-300 dark:border-gray-700">
                                @forelse($paginatedOrderedItems as $index => $item)
                                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700">
                                        <td class="px-6 py-4 border-b border-gray-300 dark:border-gray-700">{{ $item['description'] }}</td>
                                        <td class="px-6 py-4 border-b border-gray-300 dark:border-gray-700">{{ $item['item_type'] }}</td>
                                        <td class="px-6 py-4 border-b border-gray-300 dark:border-gray-700">{{ number_format($item['unit_price'], 2) }}</td>
                                        <td class="px-6 py-4 border-b border-gray-300 dark:border-gray-700">{{ number_format($item['order_qty'], 2) }}</td>
                                        <td class="px-6 py-4 border-b border-gray-300 dark:border-gray-700">{{ number_format($item['total_price'], 2) }}</td>
                                ...
                                @empty
                                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700">
                                        <td colspan="6" class="px-6 py-8 text-center border-b border-gray-300 dark:border-gray-700">
        ...
        <div class="fixed bottom-0 right-0 p-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 w-full">
            <div class="flex justify-end">
                <button type="button" wire:click="submit"
                    class="inline-flex items-center px-8 py-3 text-base font-semibold rounded-xl shadow-md transition bg-blue-700 hover:bg-blue-800 text-white focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 disabled:opacity-50 disabled:cursor-not-allowed"
