        ...
        <x-collapsible-card title="Purchase Order Receiving Report" open="true" size="full">
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div class="flex flex-col items-center justify-center p-6 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">QR Code for Stock-In</h4>
                    <div class="bg-white p-4 rounded-lg shadow-sm">
                        {!! QrCode::size(150)->generate($purchaseOrder->po_num) !!}
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 font-mono">{{ $purchaseOrder->po_num }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 text-center">Scan this QR code in the Stock-In module</p>
                </div>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @forelse($purchaseOrder->supplyOrders as $order)
                            ...
                            @if($order->supplyProfile->itemClass && $order->supplyProfile->itemClass->name === 'consumable' && $order->supplyProfile->supplyBatches->isNotEmpty())
