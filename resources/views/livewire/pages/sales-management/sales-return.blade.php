...
<div>
    <div>
        <!-- New Request Button -->
        @if(auth()->user()->can(\App\Enums\Enum\PermissionEnum::CREATE_SALES_RETURN->value))
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-1">
                        Sales Return Qualification Status
                    </h3>
                    <p class="text-xs text-gray-600 dark:text-gray-400">
                        Currently showing {{ count($salesOrders ?? []) }} sales orders that qualify for returns
                    </p>
                </div>
        ...
        <div 
            ...
            <x-modal wire:model="showCreateModal" class="max-h-[80vh] w-full max-w-2xl">
                ...
                <form wire:submit.prevent="submit" class="space-y-6">
                    ...
                    @if($editValue)
                        <i>The sales order selection is disabled for editing.</i> 
                    @endif
                    ...
                    <div class="grid gap-6 mt-5 mb-2 md:grid-cols-2">                  
                        ...
                        @else
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sales Order #</label>
                                    <button type="button" wire:click="refreshQualifiedSalesOrders" 
                                ...
                                <select wire:model.live="sales_order_id" 
                                    ...
                                    @foreach($salesOrders ?? [] as $id => $label)
                                        <option value="{{ $id }}">{{ $label }}</option>
                                    @endforeach
                                </select>
                                @if(empty($salesOrders))
                                    <p class="text-xs text-amber-600 dark:text-amber-400 mt-1">
                                        No sales orders qualify for returns. Only shipped, delivered, sold, or released orders can be returned.
                                    </p>
                                @endif
                            </div>
                        @endif 
                                        ...
                                        <td class="border p-1 text-center">
                                            ...
                                            <div class="flex items-center justify-center">
                                                <div class="bg-gray-200 p-4 rounded">
                                                    ...
                                                    @if(
                                                        auth()->user()->can(\App\Enums\Enum\PermissionEnum::EDIT_SALES_RETURN->value) || 
                                                        auth()->user()->can(\App\Enums\Enum\PermissionEnum::CREATE_SALES_RETURN->value)
                                                    )
                            ...
                            <tbody>
                                ...
                                @forelse ($salesReturns as $data)
                                    <tr wire:key
                                        ...
                                        <td class="px-6 py-4">     
                                            @if(auth()->user()->can(\App\Enums\Enum\PermissionEnum::EDIT_SALES_RETURN->value))                  
