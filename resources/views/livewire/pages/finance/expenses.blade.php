...
<x-slot:subheader>Expense Management</x-slot:subheader>
<div class="">
    <div class="">
         @can(\App\Enums\Enum\PermissionEnum::CREATE_EXPENSES->value)
        <x-collapsible-card title="Add Expense" open="false" size="full">
            <form wire:submit.prevent="save" x-show="open" x-transition>
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        <x-input type="text" wire:model.defer="reference_id" name="reference_id" label="Expenses Number" placeholder="Auto-generated" readonly class="bg-gray-100 cursor-not-allowed" />
                    ...
                    <div>
                        <x-select wire:model.defer="category" name="category" label="Category">
        ...
        <x-collapsible-card title="Expenses List" open="true" size="full">
            <div class="flex items-center justify-between p-4 pr-10">
                <div class="flex space-x-6">
                    <div class="relative">
                        <x-input type="text" wire:model.live="search" name="search" label="Search" placeholder="Search expenses..." />
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th class="px-2 py-2 whitespace-nowrap">Expense Number</th>
                    ...
                    <tbody>
                        ...
                        <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200 text-xs">
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->reference_id }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->party }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->date }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->category }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">{{ number_format($expense->amount, 2) }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->payment_method }}</td>
                            ...
                            <td class="px-2 py-2 whitespace-nowrap">{{ $expense->remarks }}</td>
                            <td class="px-2 py-2 whitespace-nowrap">
                                <div class="flex space-x-2">
                                    @can(\App\Enums\Enum\PermissionEnum::EDIT_PAYABLES->value)
                                    <x-button type="button" wire:click="edit({{ $expense->id }})" variant="warning" size="sm">Edit</x-button>
                                    @endcan
                                    @can(\App\Enums\Enum\PermissionEnum::DELETE_PAYABLES->value)
                                    <x-button type="button" wire:click="confirmDelete({{ $expense->id }})" variant="danger" size="sm">Delete</x-button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="9" class="px-2 py-2 text-center text-gray-500 whitespace-nowrap">No expenses found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <form wire:submit.prevent="update">
                        <div class="p-6 space-y-6">
                            <div class="grid gap-6 mb-6 md:grid-cols-2">
                                <div>
                                    <x-input type="text" wire:model.defer="reference_id" name="reference_id" label="Expense Number" placeholder="Auto-generated" readonly class="bg-gray-100 cursor-not-allowed" />
                                    @error('reference_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <x-input type="text" wire:model.defer="party" name="party" label="Description" placeholder="Enter Description" />
