@php
    $containerClasses = 'rounded-lg max-w-6xl mx-auto container-padding-x dark:bg-zinc-800';
@endphp
                        ...
                        @foreach($packageStatuses as $status)
                            ...
                            <tr class="h-12 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <td class="w-full pr-4">
                                    <div class="text-truncate">{{ $status['deliveryStatus'] }}</div>
                                </td>
                                ...
                                <td class="pr-4">
                                    <div class="w-32">
                                        @if ($status['deliveryStatus'] == 'Port')
                                            <div class="h-2 rounded-full bg-blue-600 transition-all transition-2s ease-in-out"></div>
                                        @endif

                                        @if ($status['deliveryStatus'] == 'Processing')
                                            <div class="h-2 rounded-full bg-yellow-600 transition-all transition-2s ease-in-out"></div>
                                        @endif

                                        @if ($status['deliveryStatus'] == 'Shipped')
                                            <div class="h-2 rounded-full bg-pink-600 transition-all transition-2s ease-in-out"></div>
                                        @endif

                                        @if ($status['deliveryStatus'] == 'Delivered')
                                            <div class="h-2 rounded-full bg-green-600 transition-all transition-2s ease-in-out"></div>
                                        @endif

                                        @if (!in_array($status['deliveryStatus'], ['Port', 'Processing', 'Shipped', 'Delivered']))
                                            <div class="h-2 rounded-full bg-red-600 transition-all transition-2s ease-in-out"></div>
                                        @endif
                                    </div>
                                </td>   
