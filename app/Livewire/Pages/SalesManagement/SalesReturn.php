...
class SalesReturn extends Component
{
    ...
    /**
     * Load sales orders that qualify for returns
     * Only orders that have been shipped, delivered, sold, or released can be returned
     */
    public function loadQualifiedSalesOrders()
    {
        $this->salesOrders = SalesOrder::whereIn('status', ['shipped', 'delivered', 'sold', 'released'])
            ->whereDoesntHave('salesReturns', function($query) {
                // Exclude orders that already have sales returns
    ...
    /**
     * Get the list of qualified sales order statuses for returns
     */
    public function getQualifiedStatuses()
    {
        return [
            'shipped' => 'Shipped - Product in transit',
            'delivered' => 'Delivered - Product received by customer',
            'sold' => 'Sold - Product sold but can be returned',
            'released' => 'Released - Order released for fulfillment'
        ];
    }

    /**
     * Refresh the qualified sales orders list
     * Useful when the component needs to update the available options
     */
    ...
    {
        return [
            'title' => 'Sales Order Return Qualification Criteria',
            'criteria' => [
                'Only sales orders with these statuses can be returned:',
                '• Shipped - Product is in transit to customer',
                '• Delivered - Product has been received by customer', 
                '• Sold - Product has been sold but can be returned',
                '• Released - Order has been released for fulfillment',
                '',
                'Orders cannot be returned if:',
                '• They are still pending, approved, or processing',
                '• They have been rejected, cancelled, or put on hold',
                '• They already have an active sales return',
                '• They have been fully returned'
            ]
        ];
    }
