    ...
    public function submitStockInReport()
    {
        try {
            ...
            foreach ($this->foundSalesOrder->items as $item) {
                ...
                // Update supply profile quantity for good items using FIFO batch deduction
                if ($itemCondition === 'good') {
                    $supplyProfile = $item->product;
                    $oldQty = $supplyProfile->supply_qty;
                    $qtyChange = $item->quantity;

                    // Use FIFO method to deduct from batches
                    $deductionResult = \App\Models\SupplyBatch::getBatchesForFifoDeduction($supplyProfile->id, $qtyChange);
                    
                    if ($deductionResult['shortage'] > 0) {
                        // Not enough stock in batches
                        $this->message = "Insufficient stock in batches for {$supplyProfile->supply_description}. Need: {$qtyChange}, Available: {$deductionResult['total_available']}";
                        $this->messageType = 'error';
                        return;
                    }
                    ...
                    $batchDeductions = [];
                    foreach ($deductionResult['batches'] as $batchData) {
                        $batch = $batchData['batch'];
                        $qtyToDeduct = $batchData['quantity'];
                        
                        if ($batch->deductQuantity($qtyToDeduct)) {
                            $batchDeductions[] = [
                                'batch_number' => $batch->batch_number,
                                'quantity_deducted' => $qtyToDeduct,
                                'remaining_qty' => $batch->current_qty
                            ];
                        } else {
                            $this->message = "Failed to deduct quantity from batch {$batch->batch_number}";
                            $this->messageType = 'error';
                            return;
                        }
                    }

                    // Update supply profile total quantity
                    ...
                    // Custom Spatie activity log with batch details
                    if ($qtyChange > 0) {
                        $batchDetails = collect($batchDeductions)->map(function($bd) {
                            return "Batch {$bd['batch_number']}: -{$bd['quantity_deducted']} (Remaining: {$bd['remaining_qty']})";
                        })->implode(', ');
                        ...
                        activity('Stock-out')
                            ...
                            ->log("Stock-out {$supplyProfile->supply_description} using FIFO: {$oldQty} → " . ($oldQty - $qtyChange) . " | {$batchDetails}");
                    }
                }
            }
