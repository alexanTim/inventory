...
class Index extends Component
{
    ...
    public $itemDONumbers = [];
    public $itemExpirationDates = [];
    public $itemManufacturedDates = [];
    public $itemLocations = [];
    public $itemBatchNotes = [];
    public $received_qty = []; // New array to hold received quantities
    
    public $checkifPartial = false;
    ...
    public function processScannedCode($code)
    {
        ...
        // Try to decode JSON first (for QR codes with structured data)
        $decodedData = null;
        ...
        try {
            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($decodedData)) {
                Log::info("Successfully decoded JSON QR code data: " . json_encode($decodedData));
                
                // Extract PO number from JSON data
                if (isset($decodedData['po_num'])) {
                    $extractedPONumber = $decodedData['po_num'];
                    Log::info("Extracted PO number from JSON: {$extractedPONumber}");
                }
            }
        } catch (Exception $e) {
            Log::info("Failed to decode JSON, treating as plain text: " . $e->getMessage());
        }
        
        // Use extracted PO number if available, otherwise use the original code
        $searchCode = $extractedPONumber ?: $code;
        Log::info("Searching with code: {$searchCode} (original: {$code})");
        
        // First, try to find a purchase order with this PO number
        $purchaseOrder = PurchaseOrder::with(['supplyOrders.supplyProfile'])
            ->where('po_num', $searchCode)
            ->first();
        ...
        if ($purchaseOrder) {
            ...
            $validStatuses = ['to_receive', 'for_delivery', 'pending','partially received'];
            ...
            $this->foundSupplyProfile = null;
            $this->showResult = true;
            if ($extractedPONumber && $extractedPONumber !== $code) {
                $this->message = "Purchase Order found: {$purchaseOrder->po_num} (extracted from QR code data)";
            } else {
        ...
        if ($extractedPONumber && $extractedPONumber !== $code) {
            $this->message = "QR code data decoded successfully, but no purchase order found for PO number: {$extractedPONumber}. Original scanned data: {$code}";
        } else {
    ...
    public function initializeStep2Data()
    {
        ...
        foreach ($this->foundPurchaseOrder->supplyOrders as $supplyOrder) {
            ...
            if ($supplyOrder->supplyProfile->isConsumable()) {
                ...
                $this->itemDONumbers[$supplyOrder->id]  = \App\Models\PurchaseOrderDelivery::generateNewDONumber(); 
    ...
    public function generateNewDONumber($supplyOrderId)
    {                 
        $this->itemDONumbers[$supplyOrderId]  = \App\Models\PurchaseOrderDelivery::generateNewDONumber(); 
    }
    ...
    public function submitStockInReport()
    {
        try {
            ...
            DB::transaction(function () {
                // delivery 
                $DR = PurchaseOrderDelivery::create([
                    'purchase_order_id' =>  $this->foundPurchaseOrder->id, 
                    'dr_number' => \App\Models\PurchaseOrderDelivery::generateNewDONumber()                    
                ]);
                ...
                foreach ($this->foundPurchaseOrder->supplyOrders as $supplyOrder) {
                    ...
                    if ($itemCondition === 'good') {
                        ...
                        $received_qty = $this->received_qty[$supplyOrder->id] ?? 0;     
                        $currentDate = now()->toDateString();
                                          
                        if($received_qty == 0){                          
                            throw new \Exception("Received quantity for {$supplyProfile->supply_description} must be greater than zero.");
                            return;
                        }

                        // check the same PO in supply batch then get all the received qty
                        // then do $supplyOrder->order_qty - $total_received_qty
                        $total_received_qty = SupplyBatch::where('supply_order_id', $supplyOrder->id)
                            ->where('status', 'active')
                            ->whereHas('supplyOrder.purchaseOrder', function ($query) {
                                $query->where('id', $this->foundPurchaseOrder->id);
                            })
                            ->sum('received_qty');

                        $total_received_qty = $total_received_qty ?: 0;
                        
                        $remaining_qty = $supplyOrder->order_qty - $total_received_qty;

                        if($received_qty > $remaining_qty){
                            throw new \Exception("Received quantity for {$supplyProfile->supply_description} exceeds remaining quantity to be received ({$remaining_qty}).");
                            return;
                        }
                        ...
                        if ($supplyProfile->isConsumable()) {
                            ...
                            if( !empty( $expirationDate ) ){                               
                                throw new \Exception("Expiration date for {$supplyProfile->supply_description} must be set.");
                                return;
                            }
                            ...
                            $dataInsert = [
                                ...
                                'delivery_id' => $DR->id, // To be set if delivery tracking is implemented                                                  
                        ...
                        $DATA = SupplyBatch::create($dataInsert); 
                        $DATA->refresh();

                        Log::info("Created new batch {$DATA->batch_number} with received_date: {$currentDate} for stock-in operation");
                        ...
                        if ($qtyChange > 0) {
                            activity('Stock-in')
                                ...
                                ->withProperties([
                                    ...
                                    'from' => $oldQty,
                                    'to' => $oldQty + $qtyChange,
                                    'batch_number' =>  ($this->itemBatchNumbers[$supplyOrder->id] ?? null),
                                    'do_number' => ($this->itemDONumbers[$supplyOrder->id] ?? null) ,
                ...
                $poStatus = ( $checkifPartial == false ) ? 'received' : 'partially received';
